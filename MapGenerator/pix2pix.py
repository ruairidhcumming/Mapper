        ...
        for epoch in range(epochs):
            for batch_i, (imgs_A, imgs_B) in enumerate(self.data_loader.load_batch(batch_size = batch_size, datapath = datapath, nImages = nImages)):
                ...
                if batch_i % sample_interval == 0:
                    self.sample_images(epoch, batch_i,datapath = datapath)

    def sample_images(self, epoch, batch_i,datapath = ''):
        os.makedirs('images/%s' % self.dataset_name, exist_ok=True)
        r, c = 3, 3

        imgs_A, imgs_B = self.data_loader.load_data(batch_size=3, is_testing=True,datapath = datapath)
        fake_A = self.generator.predict(imgs_B)
